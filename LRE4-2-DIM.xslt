...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	xmlns:lom-rights="http://ltsc.ieee.org/xsd/LOM/imslode/ilox/any/rights"
  ...
  <xsl:template name="formatDuration">
    ...
    <xsl:if test="normalize-space($value)">
      <xsl:element name="dim:field">
        ...
        <xsl:variable name="hours">
            <xsl:choose>
                <xsl:when test="contains(substring(substring-after($value,'PT'),1,2),'H')">
                <xsl:value-of select="concat(0, substring(substring-after($value,'PT'),1,1))" />
                </xsl:when>
                <xsl:otherwise>
    ...
    <!--
        ...
        input:{date} 1987-04-09
        new date format 09/04/1987
    -->
    ...
    <xsl:template match="work:expression/work:manifestation/work:name/work:value">
      <xsl:if test="normalize-space(.)">
  		...
  		<xsl:if test="contains($theValue, 'thumbnail')">
  	        <xsl:variable name="hasThumbnailValue" select="ancestor::*[1]/ancestor::*[1]/child::*[2]"/>
  ...
  *********
  ...
  <xsl:template match="lom:metaMetadata/lom:contribute">
    <xsl:if test="normalize-space(.)">
        <xsl:variable name="metametadataContributeEntityValue" select="substring-before(substring-after(descendant::*[name()='entity'],'FN:'),' VERSION:')"/>

        <xsl:if test="(not(contains(descendant::*[name()='value'], 'scientific metadata coordinator')))">

            <!-- metametadata-contribute-entity -->
            <xsl:if test="descendant::*[name()='entity']">
                <xsl:call-template name="addLiteralDimField">
                    <xsl:with-param name="mdschema" select="'lom'"/>
                    <xsl:with-param name="element" select="'metametadata-contribute-entity'"/>
                    <xsl:with-param name="value" select="$metametadataContributeEntityValue"/>
                </xsl:call-template>
            </xsl:if>
            ...
            <xsl:variable name="metametadataContributeRoleValue" select="descendant::*[name()='value']"/>
            <xsl:if test="descendant::*[name()='value']">
                <xsl:choose>
                    <xsl:when test="$metametadataContributeRoleValue='metadata creator'">
                        <xsl:call-template name="addLiteralDimField">
                            <xsl:with-param name="mdschema" select="'lom'"/>
                            <xsl:with-param name="element" select="'metametadata-contribute-role'"/>
                            <xsl:with-param name="value">Δημιουργός</xsl:with-param>
                        </xsl:call-template>
                    </xsl:when>
                    <xsl:when test="$metametadataContributeRoleValue='metadata validator'">
                        <xsl:call-template name="addLiteralDimField">
                            <xsl:with-param name="mdschema" select="'lom'"/>
                            <xsl:with-param name="element" select="'metametadata-contribute-role'"/>
                            <xsl:with-param name="value">Επικυρωτής</xsl:with-param>
                        </xsl:call-template>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:call-template name="addLiteralDimField">
                            <xsl:with-param name="mdschema" select="'lom'"/>
                            <xsl:with-param name="element" select="'metametadata-contribute-role'"/>
                            <xsl:with-param name="value" select="$metametadataContributeRoleValue"/>
                        </xsl:call-template>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:if>
            ...
            <xsl:variable name="metametadataContributeDateValue" select="descendant::*[name()='dateTime']"/>
            <xsl:if test="descendant::*[name()='dateTime']">
                <xsl:call-template name="formatDate">
                    <xsl:with-param name="mdschema" select="'lom'"/>
                    <xsl:with-param name="element" select="'metametadata-contribute-date'"/>
                    <xsl:with-param name="value" select="$metametadataContributeDateValue"/>
                </xsl:call-template>
            </xsl:if>
        ...
        <xsl:if test="contains(descendant::*[name()='value'], 'scientific metadata coordinator')">

            <!-- metametadata-contribute-entity -->
            <xsl:if test="descendant::*[name()='entity']">
                <xsl:call-template name="addLiteralDimField">
                    <xsl:with-param name="mdschema" select="'lom'"/>
                    <xsl:with-param name="element" select="'lifecycle-contribute-entity'"/>
                    <xsl:with-param name="value" select="$metametadataContributeEntityValue"/>
                </xsl:call-template>
            </xsl:if>
            <xsl:if test="not(descendant::*[name()='entity'])">
 ...
 <!--
  ...
  <xsl:template match="lom:educational/lom:learningResourceType/lom:value">
    <xsl:call-template name="addLiteralDimField">
      <xsl:with-param name="mdschema" select="'lom'"/>
      <xsl:with-param name="element" select="'educational-learningresourcetype'"/>
      <xsl:with-param name="value" select="text()"/>
    </xsl:call-template>
  </xsl:template>
  ...
  <xsl:template match="lom:educational/lom:typicalLearningTime/lom:duration">
    <xsl:call-template name="formatDuration">
      <xsl:with-param name="mdschema" select="'lom'"/>
      <xsl:with-param name="element" select="'educational-typicallearningtime'"/>
      <xsl:with-param name="value" select="text()"/>
    </xsl:call-template>
  </xsl:template>
  ...
  <xsl:template match="lom-rights:rights/lom-rights:description/lom-rights:string">
    <xsl:call-template name="addLiteralDimField">
      <xsl:with-param name="mdschema" select="'lom'"/>
   ...
   <xsl:template match="lom-rights:rights/lom-rights:cost/lom-rights:value">
	<xsl:if test="contains(text(),'no')">
		...
		<xsl:call-template name="addLiteralDimField">
		  ...
		  <xsl:with-param name="value">sure</xsl:with-param>
		</xsl:call-template>
		<xsl:call-template name="addLiteralDimField">
		  <xsl:with-param name="mdschema" select="'lom'"/>
		  <xsl:with-param name="element" select="'rights-cc'"/>
		  <xsl:with-param name="value">Attribution-ShareAlike 3.0 Unported</xsl:with-param>
		</xsl:call-template>
		<xsl:call-template name="addLiteralDimField">
		  <xsl:with-param name="mdschema" select="'lom'"/>
		  <xsl:with-param name="element" select="'rights-cc-url'"/>
		  <xsl:with-param name="value">http://creativecommons.org/licenses/by-sa/3.0/</xsl:with-param>
		</xsl:call-template>
		<xsl:call-template name="addLiteralDimField">
		  <xsl:with-param name="mdschema" select="'lom'"/>
  ...
  <xsl:template match="lom-rights:rights/lom-rights:copyrightAndOtherRestrictions/lom-rights:value">
	<xsl:if test="contains(text(),'no')">
		<xsl:call-template name="addLiteralDimField">
		  <xsl:with-param name="mdschema" select="'lom'"/>
  ...
  <xsl:if test="lom:purpose/lom:value[contains(text(), 'discipline')]">
    ...
    <xsl:if test="normalize-space(.)">
        <xsl:variable name="hasDisciplineDValue" select="descendant::*[name()='description']"/>
        <xsl:call-template name="addLiteralDimField">
            <xsl:with-param name="mdschema" select="'lom'"/>
